  ...
  useEffect(() => {
    async function load() {
      ...
      const { data } = await supabase
        .from("roasters")
        .select("roaster_name, region, seller_type, is_pro")
      ...
      <div className="min-h-screen bg-[#F8F5F2] flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center px-6 py-20">
          <div className="max-w-md w-full text-center space-y-6">
            ...
            <div>
              <h2 className="font-serif text-2xl text-[#2A1508] mb-2">Product listed!</h2>
              <p className="text-stone-500 text-sm leading-relaxed">
                <span className="font-medium text-[#2A1508]">{savedName}</span> is now on the marketplace.
                Go to <span className="font-medium text-[#2A1508]">Batch Schedule</span> in your dashboard
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <Link
                ...
                className="bg-[#C4622D] hover:bg-[#B0561A] text-white text-sm px-6 py-3 rounded-[2px] font-medium tracking-wide transition-colors"
              >
    ...
    <div className="min-h-screen bg-[#F8F5F2] flex flex-col">
      ...
      <div className="bg-[#F8F5F2] border-b border-[rgba(42,21,8,0.07)] px-6 md:px-10 pt-10 pb-8">
        <div className="max-w-2xl mx-auto">
          ...
          <h1 className="font-serif text-3xl text-[#2A1508]">Add a Product</h1>
          ...
          <SectionCard title="Formats & Pricing">
            <div className="space-y-5">
              ...
              <button
                type="button"
                onClick={addFormat}
                className="text-xs text-[#C4622D] hover:text-[#B0561A] transition-colors"
              >
                + Add custom format
              </button>
            </div>
