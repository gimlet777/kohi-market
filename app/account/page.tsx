...
export default function AccountPage() {
  ...
  useEffect(() => {
    async function checkSession() {
      ...
      if (session?.user) {
        // Verify it's not a roaster account
        const { data: roaster } = await supabase.from("roasters").select("id").eq("id", session.user.id).maybeSingle()
        if (!roaster) {
  ...
  async function loadProfile(id: string) {
    const { data } = await supabase
      ...
      .maybeSingle()
    if (data) {
      setPreferredMethod((data as ConsumerProfile).preferred_brew_method)
      setGrinderType((data as ConsumerProfile).grinder_type)
      setSetupNotes((data as ConsumerProfile).brew_notes ?? "")
    }
  }
  ...
  async function handleAuth(e: React.FormEvent) {
    ...
    if (isLogin) {
      ...
      const { data: roaster } = await supabase.from("roasters").select("id").eq("id", data.session.user.id).maybeSingle()
      if (roaster) {
        setAuthError("This email belongs to a roaster account. Please use the Roaster Portal to sign in.")
          ...
          <div className="space-y-8">
            <div>
              ...
              <p className="text-sm text-stone-400 font-light mt-1">
                {isLogin
                  ? "Sign in to save your brewing preferences."
                  : "Create a free account to save your equipment profile."}
              </p>
            </div>
            ...
            <form onSubmit={handleAuth} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 border border-stone-200 rounded-[2px] text-sm text-[#2A1A0E] placeholder-stone-300 bg-white focus:outline-none focus:border-[#C4714A] transition-colors"
                />
              </div>
              <div>
            ...
            <p className="text-xs text-stone-400 text-center">
              ...
              <button
                ...
                className="text-[#C4714A] hover:text-[#B05E3C] transition-colors"
              >
            ...
            <form onSubmit={handleSaveProfile} className="space-y-7">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-4 py-3 border border-stone-200 rounded-[2px] text-sm text-[#2A1A0E] placeholder-stone-300 bg-white focus:outline-none focus:border-[#C4714A] transition-colors resize-none"
                />
